...
const map = {
  ...
  render(snakePointsArray, foodPoint) {
    ...
    // Пальцем в небо,
    // просто увидел что на месте еды выше в цикле объект а не элемент, а мог бы и сразу догадаться
    this.usedCells.push(foodCell); // <---------------------------------------------------------------------- foodPoint
  },
};
...
const snake = {
  ...
  /**
   * Просто померяем змейку и удалим голову, это же bodySize // <------------------------------------------
   * @return {*}
   */
  getBodySize(){
...
const game = {
  ...
  init(userSettings) {
    ...
    // Элемент для счёта. Вложенность большая, но не придумал куда бы его было правильнее запихнуть
    this.scoreEl = document.querySelector(this.config.settings.scoreSelector); // <-----------------------------------
  ...
  /**
   * Просто пушим текущий размер змейкм в элемент // <------------------------------------------
   */
  updateScore(){
    this.scoreEl.innerHTML = this.snake.getBodySize();
  },
