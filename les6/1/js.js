...
const gallery = {
  settings: {
    ...
    defaultImageSrc: 'https://via.placeholder.com/728x200.gif/c8efd4/085820?text=Image+not+found',
  },
  ...
  /**
   * Получим битую картинку, подменим ей src
   * @param image
   */
  openDefaultImage(image){
    // Объект же, просто правим свойство
    image.src = this.settings.defaultImageSrc;
  },
  /**
  ...
  createScreenContainer() {
    ...
    // image.addEventListener('error', () => this.openDefaultImage(image)); // - так тоже работает
    image.onerror = () => this.openDefaultImage(image);
